...
// import { diceFaces } from '@/modules/games/even-odd/diceFaces'
...
//             <Box className='even-odd-bg w-[368px] mx-auto md:w-[525px] rounded-[10px] h-full border-[10px] border-t-0 border-[#17a2b8] pb-8'>
//               {/* Header */}
...
import { Text } from "@/ui/primitives/typography";
import Dice from "@/modules/games/even-odd/DiceSet";
import BetDisplay from "@/modules/games/even-odd/BetDisplay";
import BetSlider from "@/modules/games/even-odd//BetSlider";
import ChipSelector from "@/modules/games/even-odd/ChipSelector";
import ResultPanel from "@/modules/games/even-odd/ResultPanel";
import ActionButtons from "@/modules/games/even-odd/ActionButtons";
import { useDiceGame } from "@/modules/games/even-odd/hooks/useDiceGame";
...
export default function EvenOddPage() {
  ...
  return (
    <main className="min-h-screen">
      <Box className="even-odd-bg w-[368px] mx-auto md:w-[525px] rounded-[10px] border-[10px] border-t-0 border-[#17a2b8] pb-8">
        {/* Header */}
        <Text className="text-white font-bold text-[20px] text-center p-5">Even-Odd Game</Text>

        {/* Dice */}
        <Dice dice={game.dice} rolling={game.rolling} />
        ...
        {game.isRolling ? (
          ...
          <ResultPanel result={game.result} isEven={game.isEven} userChoice={game.userChoice} />
        ) : (
          <>
            <BetDisplay betAmount={game.betAmount} walletBalance={game.walletBalance} />
            <BetSlider betAmount={game.betAmount} setBetAmount={game.setBetAmount} maxBet={game.maxBet} />
            <ChipSelector onSelect={(chip) => game.setBetAmount(prev => Math.min(prev + chip, game.maxBet))} maxBet={game.maxBet} />
          </>
        )}
      </Box>
      ...
      <ActionButtons
        result={game.result}
        onEven={() => game.handleUserChoice("even")}
        onOdd={() => game.handleUserChoice("odd")}
        onRebet={game.rebet}
        onNewRound={game.resetGame}
      />
    </main>
  );
}
